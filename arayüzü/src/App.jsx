...
export default function App() {
  const { loadScenario, chromaticNumber, theoreticalMin, flights, toggleFlight } = useStore();
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <button 
              onClick={() => loadScenario('high_pressure')} 
              className="px-4 py-1.5 bg-indigo-600 text-white rounded-lg text-sm font-semibold hover:bg-indigo-700 transition-all shadow-sm"
            >
              high pressure
            </button>
        ...
        <section className="bg-slate-900 text-white p-8 rounded-2xl shadow-xl">
          ...
          <div className="flex flex-wrap gap-2">
            {flights.map(f => (
              ...
              >
                <span className={`w-1.5 h-1.5 rounded-full ${f.active ? 'bg-indigo-400' : 'bg-slate-600'}`}></span>
                {f.id} <br />
                {f.aircraft_size}
              </button>
            ))}
